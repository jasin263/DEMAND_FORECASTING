...
import { useDataSources, useCreateDataSource, useRefreshDataSource, useDeleteDataSource } from '../lib/api-hooks';
...
export default function DataSourcesPage() {
  ...
  const { execute: createSource, loading: creating } = useCreateDataSource();
  const { execute: refreshSource, loading: refreshing } = useRefreshDataSource();
  const { execute: deleteSource, loading: deleting } = useDeleteDataSource();
  ...
  return (
    <FeaturePageShell
      ...
      actions={
        <button className="btn-primary text-xs py-1.5" onClick={() => setShowAddForm(true)} disabled={creating}>
          <Plus size={14} />
          Add data source
        </button>
      }
    >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button
                        ...
                        title="Refresh"
                        disabled={refreshingId === source.id || refreshing}
      ...
      <Modal open={showAddForm} onClose={() => setShowAddForm(false)} title="Add data source">
        <form onSubmit={handleAddSource} className="p-5 space-y-4">
          <div>
            ...
            <input name="name" type="text" className="input-field w-full" placeholder="e.g. ERP Sales" required />
          </div>
          <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" className="btn-secondary text-xs py-1.5" onClick={() => setShowAddForm(false)}>Cancel</button>
            <button type="submit" className="btn-primary text-xs py-1.5" disabled={creating}>
              {creating ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />} Connect
            </button>
          </div>
        </form>
      </Modal>
