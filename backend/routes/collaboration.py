...
def get_sku_collaboration(sku_id: str):
...
def create_annotation(body: dict):
    m5_data._lazy_init()
...
def approve_override(override_id: str, body: dict):
    for o in OVERRIDES:
        if o['id'] == override_id:
...
def add_thread_message(thread_id: str, body: dict):
    for t in THREADS:
        if t['id'] == thread_id:
